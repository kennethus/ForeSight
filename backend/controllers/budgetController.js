...
const createBudget = async (req, res) => {
  try {
    const { userId, name, amount, spent, earned, startDate, endDate, closed } =
      req.body;
    if (
      !userId ||
      !name ||
    ...
    if (budgetExists) {
      return res
        .status(400)
        .json({ success: false, message: "Budget Name already exists!" });
    }
...
const createManyBudgets = async (req, res) => {
  try {
    const budgets = req.body.budgets;
    if (!Array.isArray(budgets) || budgets.length === 0) {
      return res
        .status(400)
        .json({ success: false, message: "No budgets provided" });
    }
    ...
    for (const budget of budgets) {
      ...
      if (
        ...
        results.failed.push({ budget, reason: "Missing or invalid fields" });
        continue;
      }

      if (!mongoose.Types.ObjectId.isValid(userId)) {
        results.failed.push({ budget, reason: "Invalid User ID" });
        continue;
      }

      if (new Date(startDate) > new Date(endDate)) {
        results.failed.push({ budget, reason: "Start date after end date" });
        continue;
      }

      const existing = await Budget.findOne({ userId, name, closed: false });
      if (existing) {
        results.failed.push({
          budget,
          reason: "Duplicate budget name for user",
        });
        continue;
      }

      // If valid, prepare for bulk insert
      validBudgets.push({
        userId,
        name,
        amount,
    ...
    if (validBudgets.length > 0) {
      const inserted = await Budget.insertMany(validBudgets);
    ...
    res.status(201).json({
      success: true,
      message: `${results.successCount} budgets created`,
...
const closeBudget = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Budget closed successfully" });
  } catch (error) {
    res.status(500).json({
      success: false,
...
const updateBudgetAmountById = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Error updating Others budget",
      error: error.message,
    });
  }
};
...
const getOpenBudgets = async (req, res) => {
  try {
    ...
    const budgets = await Budget.find({ userId, closed: false }).sort({
      createdAt: -1,
    });
    res.status(200).json({
      success: true,
    ...
    res.status(500).json({
      success: false,
      message: "Error fetching open budgets",
      error: error.message,
    });
  }
};
...
const getBudgetByName = async (req, res) => {
  try {
    ...
    if (!budget) {
      return res
        .status(400)
        .json({ success: false, message: "Budget not found" }); // 🛑 Add return here
    }

    return res.status(200).json({
      success: true,
      message: "Budget successfully fetched",
      data: budget,
    });
  } catch (error) {
