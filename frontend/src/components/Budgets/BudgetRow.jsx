...
const BudgetRow = ({ budget, onClick }) => {
    ...
    const progressBarColor = isClosed
    ...
    return (
        ...
        >
            ...
            <div className="flex justify-between items-center mb-2">
                <h3 className={`text-lg font-semibold ${isClosed ? "text-gray-500" : "text-black"}`}>
                    {budget.name}
                </h3>
                <div className="text-right">
                    <p className={`font-bold ${isClosed ? "text-gray-500" : "text-gray-900"}`}>
                        ₱{total.toLocaleString()}
                    </p>
                    {!isClosed && (
                        <p className={`text-sm ${spentPercentage > 80 ? "text-red-600" : "text-gray-500"}`}>
                            -{spentPercentage}%
                        </p>
                    )}
                </div>
